	...
	function _get_chat_messages($chat_id) {
		...
		if ($chat_messages -> num_rows() > 0) {
			...
			foreach ($chat_messages->result() as $chat_message) {
				...
				$chat_messages_html .= '<li ' . $li_class . '><span class="chat_message_header">' . $chat_message -> chat_message_timestamp . ' by ' . $chat_message -> username . '</span><p class="message_content">' . $chat_message -> chat_message_content . 
			'</p></li>';
			}
